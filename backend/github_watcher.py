...
def _fetch_file_content() -> str:
    ...
    resp = _gh(["api", f"/repos/{REPO}/contents/{WATCHED_FILE}"])
    assert isinstance(resp, dict), f"Unexpected response: {resp}"
    encoded = resp["content"].replace("\n", "")
...
def _create_fix_pr() -> dict:
    ...
    existing = _gh(["api", f"/repos/{REPO}/contents/{WATCHED_FILE}", "-q", ".sha"])
    file_sha = existing.strip() if isinstance(existing, str) else existing.get("sha", "")

    # 4. Overwrite the file with baseline content on the fix branch
    encoded = base64.b64encode(BASELINE_CONTENT.encode()).decode()
    _gh(["api", "--method", "PUT", f"/repos/{REPO}/contents/{WATCHED_FILE}",
         "-f", f"message=velira: restore {WATCHED_FILE} to validated baseline v3.2 [{fix_id}]",
         "-f", f"content={encoded}",
         "-f", f"sha={file_sha}",
                ...
                if _pr_branch:
                    try:
                        await loop.run_in_executor(
                            None,
                            lambda: _gh(["api", "--method", "DELETE", f"/repos/{REPO}/git/refs/heads/{_pr_branch}"]),
                        )
                    except RuntimeError:
                        pass
